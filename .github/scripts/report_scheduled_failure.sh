...
# exactly one "Still failing"; every recovery posts "Recovered" and closes.
# So the current streak is the run of "Still failing" since the last
# "Recovered", plus this run (which has not commented yet), plus the opening
# run when there has been no recovery to reopen from.
streak_line() {
  ...
  parsed=$(jq -r '
    ...
    | (.rest | map(select(.body | startswith("Still failing")))) as $sf
