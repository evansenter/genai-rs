...
| *(no delta variant)* | `tool_call` | Step type only — the generic server-side tool call **MCP invocations arrive as** comes whole on `step.start`, which carries the full [`Step`]. A `step.delta` tagged `tool_call` has not been observed; it would land in `StepDelta::Unknown` and be skipped, logged at `debug`. |
