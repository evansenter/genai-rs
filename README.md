...
> **Note**: code generated by `#[tool]` references `async_trait` and
> `serde_json` from your crate's dependencies. Using `#[tool]` requires
> `use genai_rs::CallableFunction;` in the module where the macro is
> applied — the generated code calls the trait method `declaration()` —
> and again in any other module that calls `.declaration()` directly.
> (The generated `<fn_name>_declaration()` free function lets remote
> modules skip the import, but the defining module always needs it.)
