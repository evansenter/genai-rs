...
> `serde_json` from your crate's dependencies. Calling `.declaration()`
> (a trait method) requires `use genai_rs::CallableFunction;` at every
> call site; alternatively, the macro also generates a free function
> `<fn_name>_declaration()` (e.g. `get_weather_declaration()`) that needs
> no trait import.
