...
- **Model name**: Never hardcode a model id — reference the constants in `src/lib.rs`, which are the single source of truth. `tests/model_literals.rs` fails the build on any hardcoded `"gemini-<digit>"` outside them.
  ...
  | `INLINE_VIDEO_MODEL` | **Inline base64 video** — `DEFAULT_MODEL` returns 400 on inline video bytes while accepting video by URI (verified live 2026-08-10) |
  ...
  In-crate unit tests use a synthetic `"test-model"` instead: they exercise serialization round-trips and never cared which model, so they should not be model-bump sites. That does not extend to non-test code — a fallback model id that reaches the wire must be a real one.
